    ...
    <v-flex xs12 sm12 md12>
        <v-layout align-center row wrap class="programme-card" @click.stop="showLots = !showLots">
            <v-flex xs12 sm12 md2 pa-0 pt-0 pb-0>
                <v-img :src="getImages(programme.thumbnail)" aspect-ratio="1.7"></v-img>
            </v-flex>
            <v-flex xs12 sm12 md4 pl-4>
                <v-layout row wrap>
                    <v-flex xs12 sm12 md12>
                        <div class="">
                            <h3 class="headline text-uppercase font-weight-bold grey--text text--darken-4">{{ programme.name }}</h3>
                        </div>
                    </v-flex>
                </v-layout>
                <v-layout align-center row wrap>
                    <v-flex xs12 sm12 md4 class="text-md-left">
                        <span class="programme-list-icon grey--text text--darken-3 body-2 font-weight-bold"><v-icon color="#00e094">location_on</v-icon> {{ programme.city }}</span>
                    </v-flex>
                    <v-flex xs12 sm12 md4 class="text-md-left">
                        <span class="programme-list-icon grey--text text--darken-3 body-2 font-weight-bold"><v-icon color="#00e094" class="mr-1">far fa-calendar-alt</v-icon> {{ programme.date_livraison }} </span>
                    </v-flex>
                    <v-flex xs12 sm12 md4 class="text-md-left">
            ...
            <v-flex xs12 sm12 md2 class="text-md-center">
                <v-chip
                    v-for="(dispositif,i) in programme.dispositifs"
                    v-bind:key="i"
...
export default {
    name: "Programmes",
    props: ['programme', 'minimum', 'countfinal', 'user'],
    data: function() {
        return {
            showLots: false,
            becallModal: false,
